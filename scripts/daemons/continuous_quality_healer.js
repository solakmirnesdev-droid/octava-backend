...
import { flags, snapshot } from '../lib/intake.js';
import {
  applyQualityGate,
  cleanOfficialTitle,
  cleanArtistName,
...
/**
 * Nothing here writes unless asked.
 *
 * AI-TRAP: this script merges duplicate artists — it moves their songs and
 * soft-deletes one of them — and neither collection carries history. A wrong
 * merge is the hardest thing in this catalogue to put back by hand, and until
 * now it happened the moment the script was run, with no preview and no copy.
 */
const { apply: APPLY } = flags();
  ...
  for (const [normKey, entries] of Object.entries(artistBuckets)) {
    if (entries.length === 1) {
      ...
      if (doc.name !== cleanedName && cleanedName.length > 0) {
        if (APPLY) await Artist.updateOne({ _id: doc._id }, { $set: { name: cleanedName } });
      ...
      if (primary.name !== primaryCleanName && APPLY) {
      ...
      for (let i = 1; i < entries.length; i++) {
        ...
        if (!APPLY) {
          const n = await Song.countDocuments({ artist: duplicate._id });
          console.log(`  👥 [bi spojio] "${duplicate.name}" -> "${primaryCleanName}" (${n} pjesama)`);
          mergedArtists++;
          continue;
        }

        // The copy is written before the merge, not after: once the songs have
        // moved there is nothing left that says which artist they belonged to.
        const moving = await Song.find({ artist: duplicate._id }).select('_id title').lean();
        snapshot('merge-artists', [{
          duplicate: { _id: duplicate._id, name: duplicate.name },
          into: { _id: primary._id, name: primaryCleanName },
          songs: moving
        }]);
...
async function healAndAuditSongs() {
  ...
  if (bulkSongUpdates.length > 0) {
    ...
    for (let i = 0; i < bulkSongUpdates.length; i += CHUNK_SIZE) {
      ...
      /*
       * AI-TRAP: this is the write that matters. Guarding only the artist merge
       * left the whole "healing" path — titles, status, genres, rewritten
       * sheets — running on a dry run, which is the opposite of what a dry run
       * promises and worse than having none.
       */
      if (APPLY) await Song.bulkWrite(chunk, { ordered: false });
      else console.log(`  [probni prolaz] preskočeno ${chunk.length} izmjena`);
    }
